...
In TME "cut editor" a link to valid model names is available for L1Menu developers to set model names, see Figure \ref{fig:tme_model_cut} for an example of Topological trigger
(similar for AXOL1TL trigger).
...
According to the model names listed in \url{https://globaltrigger.web.cern.ch/upgrade/tme/models}, we created a subdirectory structure as follows (currently in branch "dev\_v1.26.x\_nn\_models" - \url{https://github.com/cms-l1-globaltrigger/mp7_ugt_legacy/tree/dev_v1.26.x_nn_models/firmware/hdl/payload/gtl}):
...
The dependency files "axol1tl\_trigger.dep" and "topo\_trigger.dep" should contain all HLS generated filenames (with path) for all models.\\
In Figure \ref{fig:topo_dep} an example for "topo\_trigger.dep" is shown.

\begin{figure}[htb]
\centering
\includegraphics[width=15cm]{figures/topo_dep}
\caption{Example of "topo\_trigger.dep" (for model "hh\_ele\_v1" files)}
\label{fig:topo_dep}
\end{figure}
