...
where \eta_1 and \eta_2 are represented in bins.
...
where \phi_1 and \phi_2 are represented in bins.
...
Calculation similar to "Invariant mass calculation" (see \ref{sec:gtl:calculation_delta_eta}).
...
Overview of precision for correlation cuts (an example for \eg \eg correlation):
\textit{EG-EG-Delta} used for DeltaEta and DeltaPhi LUTs
\textit{EG-EG-MassPt} used for pt and unconstrained pt LUTs (mass and two-body pt calculations)
\textit{EG-EG-Math} used for cos(DeltaPhi) and cosh(DeltaEta) LUTs (mass calculations)
\textit{EG-EG-InverseDeltaRMath} used for 1/DeltaR LUTs (mass over deltaR calculations)
\textit{EG-EG-TwoBodyPtMath} used for cos(Phi) and sin(Phi) LUTs (two-body pt calculations)

\textit{EG-EG-DeltaOverlapRemoval} is obsolete, used EG-EG-Delta
\textit{EG-EG-Mass} currently not used
\textit{EG-EG-TwoBodyPt} currently is obsolete, used EG-EG-MassPt
...
The values of the LUT are calculated by multiplying $\Delta\eta$ in bins with $\eta$ step size, rounding at precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT is the value of $\Delta\eta$ in bins.

The precision values of $\Delta\eta$, $\Delta\phi$ and $\Delta$R are identical, there is only one definition for "Delta".
...
The values of the LUT are calculated by multiplying $\Delta\phi$ in bins with $\varphi$ step size, rounding at precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT is the value of $\Delta\phi$ in bins.
...
The values in the LUT are calculated by multiplying $\Delta\eta$ in bins with $\eta$ step size, calculating cosine hyperbolic, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT for $\cosh($$\Delta\eta$$)$ is the value of $\Delta\eta$ in bins.\\
...
The values in the LUT are calculated by multiplying $\Delta\phi$ in bins with $\phi$ step size, calculating cosine, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT for $\cos($$\Delta\phi$$)$ is the value of $\Delta\phi$ in bins.
For calo muon correlations one has to use the muon step size.
...
The address of the two-dimensional LUT for $\frac{1}{$$\Delta$$R^2$$}$ are the values of $\Delta\eta$ and $\Delta\phi$ in bins.
