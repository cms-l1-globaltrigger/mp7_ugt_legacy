...
\item 3. Resolution of $\Delta\eta$ and $\Delta\varphi$:
  \begin{itemize}
  ...
  \end{itemize}
\item 4. The precision of $\Delta\eta$ and $\Delta\varphi$:
  \begin{itemize}
  \item "calo calo" = 5
  \item "muon muon" = 5
  \end{itemize}
\item 5. If 1/$\Delta$$R^2$=0 ($\Delta\eta$=0 and $\Delta\varphi$=0) then correlation cut \textit{invariant mass over $\Delta$R} is true
...
% The calculation of ${pt^2}$ in VHDL (no square root in VHDL) is done by building sum of $\cos($$$\varphi$\_1$$)$x$\cos($$$\varphi$\_2$$)$ LUT value and $\sin($$$\varphi$\_1$$)$x$\sin($$$\varphi$\_2$$)$ LUT value, multiplying the sum with 2 and \pt_1 and \pt_1 LUT values and addiding the squares of \pt_1 and \pt_1 LUT values.
...
The values in the LUT are calculated by multiplying $\varphi$ hardware index with $\varphi$ step size, calculating cosine, rounding at "TwoBodyPtMath" precision position after decimal point and multipling the result with 10\textsuperscript{\tiny{precision}} to get integer values.\\
...
The values in the LUT are calculated by multiplying $\varphi$ hardware index with $\varphi$ step size, calculating sine, rounding at "TwoBodyPtMath" precision position after decimal point and multipling the result with 10\textsuperscript{\tiny{precision}} to get integer values.\\
