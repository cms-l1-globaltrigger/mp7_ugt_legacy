...
The following cuts are for two object correlations:
\begin{itemize}
...
The generation of LUTs for calculations of correlation cuts is described in chapter "Calculation of look-up-tables (LUTs) for correlation cuts" (see \ref{sec:gtl:calc_luts_corr_cuts}).
...
\textbf{Calculation of $\Delta\phi$}
\label{sec:gtl:calculation_delta_phi}

The calculation of \textit{$\Delta\phi$} of two objects is done with formula:\\

$\Delta\phi$=abs(\phi_1-\phi_2) with ("\phi full bin range"-$\Delta\phi$) when ($\Delta\phi$ > "\phi half bin range").\\

where \phi_1 and \phi_2 are represented in hardware indices.
...
The calculation of $\Delta$$R^2$ in VHDL (no square root in VHDL) is done by adding the square of $\Delta\eta$ and $\Delta\phi$ LUT values.
...
The calculation of $\frac{M^2}{2}$ in VHDL (no square root in VHDL) is done by multiplying LUT values of \pt_1, \pt_2 and the difference of $\cosh($$\Delta\eta$$)$ and $\cos($$\Delta\phi$$)$.

% In TME there are two thresholds for M: "greater/equal lower limit" and "less/equal upper limit", given in GeV with one position after decimal point in even numbers.\footnote{even numbers to get a precision of one position after decimal point after dividion by 2, because VHDL-Producer calculates thresholds for $\frac{M^2}{2}$, which includes a division by 2.}
...
Calculation similar to "Invariant mass calculation" (see \ref{sec:gtl:inv_mass_calculation}).
...
The calculation of ${pt^2}$ in VHDL (no square root in VHDL) using LUTs for \pt, $\cos($$\varphi$$)$ and $\sin($$\varphi$$)$.

% The calculation of ${pt^2}$ in VHDL (no square root in VHDL) is done by building sum of $\cos($$\varphi_1$$)$x$\cos($$\varphi_2$$)$ LUT value and $\sin($$\varphi_1$$)$x$\sin($$\varphi_2$$)$ LUT value, multiplying the sum with 2 and \pt_1 and \pt_1 LUT values and addiding the squares of \pt_1 and \pt_1 LUT values.
...
LUTs are defined as a VHDL "constant" in \texttt{gtl_luts_pkg.vhd} (VHDL package file).
The values of precision and step size are given by "scale_set" in XML file of a L1 menu.

Overview of precision types for correlation cuts (an example for \eg \eg correlation):
\textit{EG-EG-Delta} relevant for DeltaEta and DeltaPhi LUTs
\textit{EG-EG-MassPt} relevant for pt and unconstrained pt LUTs (used in mass and two-body pt calculations)
\textit{EG-EG-Math} relevant for cos(DeltaPhi) and cosh(DeltaEta) LUTs (used in mass calculations)
\textit{EG-EG-InverseDeltaRMath} relevant for 1/DeltaR LUTs (used in mass over deltaR calculations)
\textit{EG-EG-TwoBodyPtMath} relevant for cos(Phi) and sin(Phi) LUTs (used in two-body pt calculations)

\textit{EG-EG-DeltaOverlapRemoval} is obsolete, used EG-EG-Delta (same scales for $\eta$ and $\varphi$)
\textit{EG-EG-Mass} currently not used
\textit{EG-EG-TwoBodyPt} is obsolete, used EG-EG-MassPt
...
The values of \pt or unconstrained \pt LUT are calculated by building the half difference of maximum and minimum value of a bin, adding minimum value, rounding at precision position after decimal point and multiplying with $10^precision$ to get integer values.\\
...
JET_PT_LUT
ETM_PT_LUT (used also for \htm and ET$_{miss}^{HF}$)
MU_PT_LUT
MU_UPT_LUT
...
The values of the LUT are calculated by multiplying $\Delta\eta$ in hardware indices with $\eta$ step size, rounding at precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
...
where <n_bits> is the precision value and <type> represents a precision name.\\

The $\eta$ (=$\Delta\eta$) step size in XML file is given by (an example for \eg):
\textit{<scale>
<object>EG</object>
    <type>ETA</type>
    ...
...
\textbf{LUTs for $\Delta\phi$}
\label{sec:gtl:calc_luts_delta_phi}

The values of the LUT are calculated by multiplying $\Delta\phi$ in hardware indices with $\varphi$ step size, rounding at precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT is the value of $\Delta\phi$ in hardware indices.

The precision values of $\Delta\phi$ are identical with $\Delta\eta$.

The $\phi$ (=$\Delta\phi$) step size in XML file is given by (an example for \eg):
...
The values in the LUT are calculated by multiplying $\Delta\eta$ in hardware indices with $\eta$ step size, calculating cosine hyperbolic, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
...
used for $\cosh($$\Delta\eta$$)$ and $\cos($$\Delta\phi$$)$.
...
\textbf{LUTs for $\cos($$\Delta\phi$$)$ used in mass calculations}
\label{sec:gtl:calc_luts_cos_delta_phi}

The values in the LUT are calculated by multiplying $\Delta\phi$ in hardware indices with $\phi$ step size, calculating cosine, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
The address of the LUT for $\cos($$\Delta\phi$$)$ is the value of $\Delta\phi$ in hardware indices.
For calo muon correlations one has to use the muon step size.

VHDL names of $\cos($$\Delta\phi$$)$ LUTs:
...
The calculation of $\frac{1}{$$\Delta$$R^2$$}$ is done by adding the squares of $\Delta\eta$ and $\Delta\phi$ LUT values, rounding at "InverseDeltaRMath" precision position after decimal point and multipling the result with $10^precision$ to get integer values.
The address of the two-dimensional LUT for $\frac{1}{$$\Delta$$R^2$$}$ are the values of $\Delta\eta$ and $\Delta\phi$ in hardware indices.
...
% \textbf{LUTs for \pt used two-body pt cuts}
%
% The values of \pt LUT are calculated by building the difference of maximum and minimum value of a bin, rounding at precision position after decimal point and multiplying with $10^precision$ to get integer values.\\
% The address input of the LUT for \pt is the value of hardware index of \pt.
%
% The precision values in XML file are given by (an example for \eg \eg correlation):
% \textit{<scale>
%     <object>PRECISION</object>
%     <type>EG-EG-TwoBodyPt</type>
%     ...
%     <n_bits>1</n_bits>
% </scale>}\\
...
The values in the LUT are calculated by multiplying $\phi$ hardware index with $\phi$ step size, calculating cosine, rounding at "TwoBodyPtMath" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
...
The values in the LUT are calculated by multiplying $\phi$ hardware index with $\phi$ step size, calculating sine, rounding at "TwoBodyPtMath" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
