...
LUTs are provided for calo-calo, calo-muon and muon-muon correlations. They are defined as "constant" in a VHDL package file (gtl_luts_pkg.vhd) and named:
CALO_CALO_DIFF_ETA_LUT
CALO_MU_DIFF_ETA_LUT
MU_MU_DIFF_ETA_LUT

The address of the LUT is the value of $\Delta\eta$ in bins. The values in the LUT are calculated by multiplying $\Delta\eta$ in bins with $\eta$ step size, rounding at precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
...
LUTs for $\Delta\phi$ values are created and used similar to those for $\Delta\eta$ and named:
CALO_CALO_DIFF_PHI_LUT
CALO_MU_DIFF_PHI_LUT
MU_MU_DIFF_PHI_LUT

The precision values of $\Delta\phi$ are identical with $\Delta\eta$ (see \ref{sec:gtl:calculation_delta_eta}).
...
The calculation of $\frac{M^2}{2}$ in VHDL is based on LUTs for \pt, $\cosh($$\Delta\eta$$)$ and $\cos($$\Delta\phi$$)$.
Names for \pt LUTs:
EG_PT_LUT (= TAU_PT_LUT)
JET_PT_LUT
ETM_PT_LUT (= HTM_PT_LUT, ETMHF_PT_LUT)
MU_PT_LUT
MU_UPT_LUT
Names for $\cosh($$\Delta\eta$$)$ LUTs:
CALO_CALO_COSH_DETA_LUT
CALO_MUON_COSH_DETA_LUT
MU_MU_COSH_DETA_LUT
Names for $\cos($$\Delta\phi$$)$ LUTs:
CALO_CALO_COS_DPHI_LUT
CALO_MUON_COS_DPHI_LUT
MU_MU_COS_DPHI_LUT

The address of the LUT for \pt is the value of hard ware value of \pt.
The values in the LUT are calculated with maximum value minus minimum value of a bin multipled with $10^precision$ to get integer values.\\
...
The address of the LUT for $\cosh($$\Delta\eta$$)$ is the value of $\Delta\eta$ in bins. The values in the LUT are calculated by multiplying $\Delta\eta$ in bins with $\eta$ step size, calculating cosine hyperbolic, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\

The address of the LUT for $\cos($$\Delta\phi$$)$ is the value of $\Delta\phi$ in bins. The values in the LUT are calculated by multiplying $\Delta\phi$ in bins with $\phi$ step size, calculating cosine, rounding at "Math" precision position after decimal point and multipling the result with $10^precision$ to get integer values.\\
...
for $\cosh($$\Delta\eta$$)$ and $\cos($$\Delta\phi$$)$.
...
\paragraph{LUTs for \pt and unconstrained \pt}
\label{sec:gtl:luts_pt_upt}

\paragraph{LUTs for $\Delta\eta$ and $\Delta\phi$}
\label{sec:gtl:luts_deta_dphi}

\paragraph{LUTs for $\cosh($$\Delta\eta$$)$}
\label{sec:gtl:luts_cosh_deta}

\paragraph{LUTs for $\cos($$\Delta\varphi$$)$}
\label{sec:gtl:luts_cos_dphi}
