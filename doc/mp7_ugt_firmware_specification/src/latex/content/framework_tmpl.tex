\section{Framework}\label{sec:frame:framework}
...
The central data type of Framework is shown in Listing \ref{lst_lhc_data_t} (see Section \ref{section_lhc_data_pkg} for details). In the current configuration it comprises
...
\lstinputlisting[label=lst:frame_vhd,language=VHDL,caption=Entity declaration of \texttt{frame.vhd}]{interfaces/frame.vhd}
...
\subsection{Lane Mapping Process} \label{sec:frame:lmp}
...
\subsection{SIM and SPY Memory}\label{sec:frame:sim-spy}
\textbf{Remark:}\\
...
\subsubsection{Errors}\label{subsec:frame:tcmerrors}
...
\subsection{Software Reset} \label{sec:frame:software_reset}
