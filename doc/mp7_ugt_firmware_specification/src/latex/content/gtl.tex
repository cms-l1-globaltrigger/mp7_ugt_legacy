...
\begin{itemize}
\item A top-of-hierarchy file (\texttt{gtl\_module.vhd}), which contains the pipeline for $\pm$2bx data, the instantiations of calculators for differences in $\eta$ and $\varphi$, the instantiations of conditions, the instantiations of charge correlation logic of muons and the Algorithms logic for 512 Algorithms, as well as a package file (\href{\gitbranch/tree/master/firmware/hdl/packages/gtl_pkg.vhd}{\texttt{gtl\_pkg.vhd}}) for declarations.
...
These snippets are inserted into templates for \texttt{gtl\_module.vhd} (\href{\gitbranch/tree/master/firmware/hdl/payload/gtl/gtl_module_tpl.vhd}{\texttt{gtl\_module\_tpl.vhd}}), \texttt{algo\_mapping\_rop.vhd} (\href{\gitbranch/tree/master/firmware/hdl/payload/fdl/algo_mapping_rop_tpl.vhd}{\texttt{algo\_mapping\_rop\_tpl.vhd}}) and \texttt{fdl\_pkg.vhd} (\href{\gitbranch/tree/master/firmware/hdl/packages/fdl_pkg_tpl.vhd}{\texttt{fdl\_pkg\_tpl.vhd}}) during simulation and synthesis (see Figure \ref{fig:gtl:tme_gtl}).
...
All the declarations for arrays ('type'), parameters ('constant') and look-up-tables ('constant') used in modules are available in \href{\gitbranch/tree/master/firmware/hdl/packages/gtl_pkg.vhd}{\texttt{gtl\_pkg.vhd}} package-file.
...
For calo-calo mass over deltaR 5 ROMs are needed to represend the LUT. The content of the LUT one can see in \href{\gitbranch/tree/master/firmware/hdl/ngc/blk_mem_gen_v8_4_4/lut_calo_inv_dr_sq_rom1.coe}{\texttt{lut\_calo\_inv\_dr\_sq\_rom1.coe}} (and so on).\\
For muon-muon mass over deltaR 6 ROMs are needed to represend the LUT. The content of the LUT one can see in \href{\gitbranch/tree/master/firmware/hdl/ngc/blk_mem_gen_v8_4_4/lut_muon_inv_dr_sq_rom1.coe}{\texttt{lut\_muon\_inv\_dr\_sq\_rom1.coe}} (and so on).\\
The access to a certain ROM depends on the values of $\Delta\eta$ and $\Delta\varphi$ (see \href{\gitbranch/tree/master/test_rom_lut_calo_inv_dr_sq_all/doc/description_rom_lut_calo_inv_dr_sq.txt}{\texttt{description\_rom\_lut\_calo\_inv\_dr\_sq.txt}} and \href{\gitbranch/tree/master/firmware/hdl/payload/gtl/common/rom_lut_calo_inv_dr_sq_all.vhd}{\texttt{rom\_lut\_calo\_inv\_dr\_sq\_all.vhd}}, respectively\\ 
...
\href{\gitbranch/tree/master/firmware/hdl/payload/gtl/common/rom_lut_muon_inv_dr_sq_all.vhd}{\texttt{rom\_lut\_muon\_inv\_dr\_sq\_all.vhd}}).\\
...
For the entity-declaration of \href{\gitbranch/tree/master/firmware/hdl/gt_mp7_core/gtl_fdl_wrapper/gtl/esums_conditions.vhd}{\texttt{esums\_conditions.vhd}}, see Listing~\ref{lst:gtl:esums_conditions_vhd}.
...
\label{sec:gtl:adt}
The possibility of implementing "Anomaly detection Trigger" is forseen in this version of \ugtl. Therefore a VHDL entity is defined. The VHDL code for "Anomaly detection Trigger" is provided by groups outside of \gt for synthesis of firmware.
