...
\end{bytefield}
\end{center}
% \tiny{Remark: "AD I" means Anomaly Detection Integer part, "AD D" means Anomaly Detection Decimal part, "l" means least significant bits, "h" means most significant bits, "HI" means Heavy Ion bits}\normalsize
...
% \begin{center}
% \begin{bytefield}[boxformatting={\centering\itshape}, bitwidth=1.2em, endianness=big]{28}
%         \bitheader{0,10,11,17,18,18,19,26,27,27} \\
%         \bitbox {1}     {\texttt{F}} &
%         \bitbox {8}     {\texttt{$\varphi$}} &
%         \bitbox {1}     {\texttt{S}} &
%         \bitbox {7}     {\texttt{$\eta$}}  &
...
% \end{bytefield}
% \end{center}
% \tiny{Remark: "S" means side (eta sign bit), "F" means flag.}\normalsize
...
% The calorimeter layer 1 trigger processing identifies \textbf{Boosted jets (bjet)}, \textbf{Anomaly Detection} and a \textbf{Heavy Ion bits}.\\
...
See also \ref{sec:gtl:calo1_optical_interfaces}.\\
% Six bjet objects, one value for Anomaly Detection and 8 Heavy Ion bits are passed to the \ugt for each event.\\
% For each selected bjet object, the Calo-Layer1 sends parameters for \pt, for position and a flag - encoded in 28 bits:
...
\label{sec:gtl:adt}
The possibility of implementing "Anomaly Detection Trigger" (AXOL1TL) is forseen in this version of \ugtl. The VHDL code is given by "Anomaly Detection Trigger" group in ../firmware/hdl/payload/gtl/anomaly\_detection with \href{\gitbranch/firmware/hdl/payload/gtl/anomaly_detection/axol1tl_v3.vhd}{\texttt{axol1tl\_v3.vhd}} as top module. In \href{\gitbranch/firmware/hdl/payload/gtl/anomaly_detection/adt_wrapper.vhd}{\texttt{adt\_wrapper.vhd}} module axol1tl\_v3.vhd is instantiated and a comparison of the output of axol1tl\_v3.vhd ("anomaly\_score") with a given threshold is implemented to get an "Anomaly Detection Trigger". The "anomaly\_score" consists of 18 bits, currently.

\subsubsection{Topological Trigger}
\label{sec:gtl:topo}
Similar to "Anomaly Detection Trigger" a "Topological Trigger" is part of this version. The VHDL code is given by "Topological Trigger" group in ../firmware/hdl/payload/gtl/topo\_trigger with \href{\gitbranch/firmware/hdl/payload/gtl/topo_trigger/topo_trigger.vhd}{\texttt{topo\_trigger.vhd}} as top module. In \href{\gitbranch/firmware/hdl/payload/gtl/topo_trigger/topo_wrapper.vhd}{\texttt{topo\_wrapper.vhd}} module topo\_trigger.vhd is instantiated and a comparison of the output of topo\_trigger.vhd ("topo\_score") with a given threshold is implemented to get a "Topological Trigger". The "topo\_score" consists of 16 bits, currently.

\subsubsection{CICADA Trigger}
\label{sec:gtl:cicada}
The Anomaly Detection value (of CICADA) is represented by an 8 bits integer part and an 8 bits decimal part, which are combined to a 16 bits word for comparison with requested threshold ("cscore"). In TME~\cite{TME_repo} the threshold can be selected with predefined values for the decimal part (x/2**8, for all x from 0 to 255) and values from 0 to 255 for the integer part. In VHDL Producer the threshold is converted to integer by multiplying with 2**cicada\_decimal\_precision (cicada\_decimal\_precision = number of bits of decimal part) for "cscore" value in VHDL module \href{\gitbranch/firmware/hdl/payload/gtl/cicada_condition.vhd}{\texttt{cicada\_condition.vhd}}. The trigger decision is made by comparison of 16 bits Anomaly Detection value (of CICADA) with "cscore".
