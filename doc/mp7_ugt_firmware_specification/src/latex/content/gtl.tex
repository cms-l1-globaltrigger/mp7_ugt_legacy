...
The main part of this data contains Boosted jets (bjets). Calo-Layer1 sends
6 bjets (28 bits), each on one of the 6 frames in a 240MHz domain.
In addition Anomaly Detection with an integer and a decimal part and Heavy Ion bits
are provided.\\
...
\item 8 (7+1 sign) bits pseudo-rapidity ($\eta$) position, range = -5.0 to 5.0, step = 0.087/2, linear scale, 230 bins (HW index = 0x8D..0x72)
\item 8 bits azimuth angle ($\varphi$) position, range = 2$\pi$, step $\approx$ 2$\pi$/144 (\^=2.5°), 144 bins (HW index = 0..0x8F), HW index starting at 0° (anti-clockwise)
\item 1 bit as a flag
\end{itemize}

The Anomaly Detection value is represented by an 8 bits integer part and an 8 bits decimal part. There are 8 Heavy Ion bits.
