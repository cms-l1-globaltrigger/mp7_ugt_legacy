...
6.8 & Updated text of section "Implementation in firmware" for Framework (\ref{sec:frame:implementation_firmware}), GTL (\ref{sec:gtl:implementation_firmware_gtl}) and FDL (\ref{sec:fdl:implementation_firmware}). & 2022/02/10\\
...
4.5 & Updated text in section "Framework" (\ref{sec:frame:framework}) according to firmware version v1.2.3 of frame.vhd. & 2018/01/19\\
...
Inserted link to "Scales for inputs to $\mu$GT" (\ref{sec:gtl:implementation_firmware_gtl}). Moved section "Software reset" to section "Framework" as subsection (\ref{sec:frame:software_reset}).
...
Updated sections "Firmware" (\ref{sec:fw:fw}), "Framework" (\ref{sec:frame:framework}) and "Final Desicion Logic" (\ref{sec:fdl:ufdl}). & 2016/06/09\\
...
3.1 & Updated Figure \ref{fig:fdl:mFDL_firmware} and Tables \ref{tab:ufdl_register_map}. Remaned
...
1.9 & Minor changes "Demux Lane Data" (see \ref{sec:frame:demux_lane_data}) and "Muon data" (see \ref{sec:gtl:muon_data}). & 2014/11/06\\
1.8 & Edited Section "Energy sum quantities conditions" (see \ref{sec:gtl:esums_conditions}). & 2014/10/08\\
1.7 & Added sections "Configuration of optical connections" (\ref{sec:frame:sec_configuration_optical_conn}), "Demux Lane Data" (\ref{sec:frame:demux_lane_data})
and "Lane Mapping Process" (\ref{sec:frame:lmp}) to framework. Removed tables of optical interfaces from gtl and referenced to tables in framework. & 2014/10/07\\
