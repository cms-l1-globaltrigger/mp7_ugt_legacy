% ------------------------------------------------------------------------------
% Repository path   : $HeadURL: https://hbergauer@forge.hephy.oeaw.ac.at/scm/svn/project-cmstrigger/GlobalTriggerUpgrade/doc/latex/gt-mp7-firmware-specification/content/gtl.tex $
% Last committed    : $Revision: 4487 $
% Last changed by   : $Author: hbergauer $
% Last changed date : $Date: 2018-08-14 14:38:04 +0200 (Tue, 14 Aug 2018) $
% Description       : Specification of Global Trigger Logic.
% ------------------------------------------------------------------------------
...
\begin{bytefield}[boxformatting={\centering\itshape}, bitwidth=1.0em, endianness=big]{36}
        \bitheader{0,9,10,18,19,22,23,31,32,33,34,35} \\
        \bitbox {2}     {\small  \texttt{ch}}       &
        \bitbox {2}     {\small \texttt{iso}}       &
        \bitbox {9}     {\texttt{$\eta$}}       &
        \bitbox {4}     {\texttt{qual}}       &
        \bitbox {9}     {\texttt{\pt}}    &
        \bitbox {10}    {\texttt{$\varphi$}} \\
\end{bytefield}
\end{center}
...
The selection of the mode of \et-comparator (greater/equal or equal), the \et-threshold-value, ranges for $\eta$ and $\varphi$ set with thresholds, LUTs for isolation and
...
\item "Double objects requirements condition over bx":
    ...
    \end{itemize}
% \item "Double objects requirements condition with spatial correlation over bx":
%     \begin{itemize}
%     \item Two requirements with different bx => One "correlation condition with different calorimeter object types" used, but with same object types from different "relative bx".
%     \end{itemize}
\end{itemize}

\subparagraph{Calorimeter conditions module - version 7 "no quad" and version 6 "quad"}

Two module for conditions with calorimeter objects (\texttt{calo\_conditions\_v7\_no\_quad.vhd} and \texttt{calo\_conditions\_v6\_quad.vhd}) instantiates the calorimeter comparators module (\texttt{calo\_comparators\_v2.vhd}) as many times as
...
In the calorimeter conditions module version 7 "no quad" a cut for two-body pt calculation can be selected (see ~\ref{sec:gtl:twobody_pt_calculation}). Therefore a threshold value two-body pt is required.\\
In the calorimeter conditions module version 6 "quad" the architecture is different to prevent "segmentation faults" in Vivado, because of a to big array in VHDL code. The solution is to use
smaller arrays in parallel.

%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:calo_conditions_vhd,language=VHDL,caption=Entity declaration of \texttt{calo\_conditions\_v7\_no\_quad.vhd}]{interfaces/calo_conditions_v7_no_quad.vhd}
...
\verb|eta_full_range| & array of four boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore| & array of four boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\subparagraph{Calorimeter conditions module - version 7 "no quad" and version 6 "quad" - template for VHDL-Producer}
...
instantiating a calorimeter condition (\texttt{calo\_conditions\_v7\_no\_quad.vhd} and \texttt{calo\_conditions\_v6\_quad.vhd}).\\

\subparagraph{Calorimeter comparators module - version 2}\label{sec:gtl:calo_comp_module}
A comparator between the energy (\et) and a threshold (et\_threshold) and a comparison in $\eta$ and $\varphi$ with two "window"-comparators each
...
The comparison in $\eta$ is done with two "window"-comparators, so one gets two ranges for $\eta$. The $\eta$ value (HW index) has a Two's Complement notation, the comparisons is done signed.\\
There are two flags, one for "full-range" and one for "ignore-second-window" for the selection of the ranges. The flags are given for $\eta$ and also for $\varphi$.\\
The comparison in $\varphi$ is done with two "window"-comparators, so one gets two ranges for $\varphi$. The comparisons is done unsigned.\\
There are 2 cases how the limits of one "window"-comparator could be set (see also Figure~\ref{fig:gtl:phi_windows_comparator} and Listing~\ref{lst:phi_window_comparator_vhd}):
\begin{itemize}
...
\subparagraph{Muon conditions module - version 7}
A module for conditions with muon objects (\texttt{muon\_conditions\_v7.vhd}) instantiates the muon comparators module (\texttt{muon\_comparators\_v2.vhd}) as many times as
the numbers of objects and requirements determine. Depending on the condition-type different and-or-structures of object vs. requirement are selected.
...
\clearpage
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:muon_conditions_vhd,language=VHDL,caption=Entity declaration of \texttt{muon\_conditions\_v7.vhd}]{interfaces/muon_conditions_v7.vhd}
...
\subparagraph{Muon conditions module - version 7 - template for VHDL-Producer}
...
instantiating a muon condition (\texttt{muon\_conditions\_v7.vhd}).\\

\subparagraph{Muon comparators module - version 2}\label{sec:gtl:muon_comp_module}

A comparator between \pt and a threshold (pt\_threshold), a comparison in $\eta$ and $\varphi$ with two "window"-comparators each, a comparison of quality with LUT
...
\begin{itemize}
\item Correlation condition with calorimeter objects\\(\texttt{calo\_calo\_correlation\_condition\_v4.vhd}:
\egamma-\egamma, \egamma-jet, \egamma-tau, jet-jet, jet-tau and tau-tau are possible.)
\item Correlation condition with calorimeter objects and muons objects\\(\texttt{calo\_muon\_correlation\_condition\_v3.vhd}: \egamma-muon, jet-muon and tau-muon are possible.)
\item Correlation condition with muon objects\\(\texttt{muon\_muon\_correlation\_condition\_v4.vhd})
\item Correlation condition with calorimeter objects and \esums (\etm, ET$_{miss}^{HF}$ and \htm only)\\(\texttt{calo\_esums\_correlation\_condition\_v3.vhd}: \egamma-etm, jet-etm, tau-etm,
\egamma-htm, jet-htm, tau-htm, \egamma-etmhf, jet-etmhf and tau-etmhf are possible.)
\item Correlation condition with muon objects and \esums (\etm, ET$_{miss}^{HF}$ and \htm only)\\(\texttt{muon\_esums\_correlation\_condition\_v3.vhd}: muon-etm, muon-etmhf and muon-htm are possible.)
\end{itemize}

\subparagraph{Calo Calo Correlation condition module - Version 4}
...
generic list of \texttt{calo\_calo\_correlation\_condition\_v4.vhd} named "same\_bx").\\
...
For the VHDL entity declaration of calo calo correlation condition module (version 4) in \texttt{calo\_calo\_correlation\_condition\_v4.vhd}, see Listing~\ref{lst:calo_calo_correlation_condition_vhd}.\\
\clearpage
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:calo_calo_correlation_condition_vhd,language=VHDL,caption=Entity declaration of \texttt{calo\_calo\_correlation\_condition\_v4.vhd}]{interfaces/calo_calo_correlation_condition_v4.vhd}
...
\verb|eta_full_range_calo1| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_calo1| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\verb|eta_full_range_calo2| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_calo2| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\subparagraph{Calo Muon Correlation condition module - Version 3}
...
For the VHDL entity declaration of calo muon correlation condition module (version 3) in \texttt{calo\_muon\_correlation\_condition\_v3.vhd}, see Listing~\ref{lst:calo_muon_correlation_condition_vhd}.\\
\clearpage
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:calo_muon_correlation_condition_vhd,language=VHDL,caption=Entity declaration of \texttt{calo\_muon\_correlation\_condition\_v3.vhd}]{interfaces/calo_muon_correlation_condition_v3.vhd}
...
\verb|eta_full_range_calo| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_calo| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\verb|eta_full_range_muon| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_muon| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\subparagraph{Muon Muon Correlation condition module - Version 4}
...
generic list of \texttt{muon\_muon\_correlation\_condition\_v4.vhd} named "same\_bx"). In the case of a "Double objects requirement condition", requirements for "requested charge correlations"
...
For the VHDL entity declaration of muon muon correlation condition module in \texttt{muon\_muon\_correlation\_condition\_v4.vhd}, see Listing~\ref{lst:muon_muon_correlation_condition_vhd}.\\
\clearpage
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:muon_muon_correlation_condition_vhd,language=VHDL,caption=Entity declaration of \texttt{muon\_muon\_correlation\_condition\_v4.vhd}]{interfaces/muon_muon_correlation_condition_v4.vhd}
...
\verb|eta_full_range_muon1| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_muon1| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\verb|eta_full_range_muon2| & boolean to set full range of $\eta$.\\
...
\verb|eta_w2_ignore_muon2| & boolean to ignore "window"-comparator 2 for $\eta$.\\
...
\verb|diff_eta| & differences in $\eta$, calculated in an instance of module \texttt{sub\_eta\_integer\_obj\_vs\_obj.vhd} in top-of-hierarchy module (\texttt{gtl\_module.vhd}), see \ref{sec:gtl:calculation_differences}.\\
\verb|diff_phi| & differences in $\varphi$, calculated in an instance of module \texttt{sub\_phi\_integer\_obj\_vs\_obj.vhd} in top-of-hierarchy module (\texttt{gtl\_module.vhd}).\\
\verb|pt1| & calorimeter \et values [from LUT, in $GeV\times$10].\footnote{value 10 from 10\textsuperscript{\tiny{CALO\_MUON\_INV\_MASS\_PT\_PRECISION}}}\\
\verb|pt2| & muon \pt values [from LUT, in $GeV \times$10].\\
\verb|cosh_deta| & $\cosh(\Delta\eta)$ values [from LUT, $\cosh(\Delta\eta)\times$10000].\footnote{value 10000 from 10\textsuperscript{\tiny{CALO\_MUON\_INV\_MASS\_COSH\_COS\_PRECISION}}}\\
\verb|cos_dphi| & $\cos(\Delta\varphi)$ values [from LUT, $\cosh(\Delta\varphi)\times$10000].\\
...
\subparagraph{Calo Esums Correlation condition module - Version 3}
...
For VHDL entity declaration for calo esums correlation condition module in \texttt{calo\_esums\_correlation\_condition\_v3.vhd}, see Listing~\ref{lst:calo_esums_correlation_condition_vhd}.\\
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:calo_esums_correlation_condition_vhd,language=VHDL,caption=Entity declaration of \texttt{calo\_esums\_correlation\_condition\_v3.vhd}]{interfaces/calo_esums_correlation_condition_v3.vhd}
...
\subparagraph{Muon Esums Correlation condition module - Version 3}
...
For VHDL entity declaration for muon esums correlation condition module in \texttt{muon\_esums\_correlation\_condition\_v3.vhd}, see Listing~\ref{lst:muon_esums_correlation_condition_vhd}.\\
%% "automatic generation" of listings (see /scripts/extract_entities.sh)
\lstinputlisting[label=lst:muon_esums_correlation_condition_vhd,language=VHDL,caption=Entity declaration of \texttt{muon\_esums\_correlation\_condition\_v3.vhd}]{interfaces/muon_esums_correlation_condition_v3.vhd}
...
% \end{itemize}
% \begin{itemize}
% \item \textit{{\%- include "subTemplates/instance\_calo\_condition\_v3.vhd.j2"\%}} => replace with strings defined in template:\\
% \texttt{instance\_calo\_condition\_v3.vhd.j2}.\\
% \item \textit{{\%- include "subTemplates/instance\_muon\_condition\_v3.vhd.j2"\%}} => replace with strings defined in template:\\
% \texttt{instance\_muon\_condition\_v3.vhd.j2}.\\
...
% 
\subsubsection{Calorimeter conditions - version 7 "no quad" and version 6 "quad" - template for VHDL-Producer}\label{sec:gtl:calo_conditions_tme}
A VHDL-template for VHDL-Producer of instantiating\\ \texttt{calo\_conditions\_v7\_no\_quad.vhd} \\ and \\ \texttt{calo\_conditions\_v6\_quad.vhd}\\ is given below (see Listing~\ref{lst:instance_calo_condition_vhd}).\\
...
\verb|etaFullRange| & array to set full range of $\eta$ (4x boolean).\\
...
\verb|etaW2Ignore| & array to ignore "window"-comparator 2 of $\eta$ (4x boolean).\\
...
\subsubsection{Muon conditions - version 7 - template for VHDL-Producer}
A VHDL-template for VHDL-Producer of instantiating\\ \texttt{muon\_conditions\_v7.vhd}\\ is given below (see Listing~\ref{lst:instance_muon_condition_vhd}).\\
...
\subsubsection{Calo Calo Correlation condition - Version 4 - template for VHDL-Producer}
A VHDL-template for VHDL-Producer of instantiating\\ \texttt{calo\_calo\_correlation\_condition\_v4.vhd}\\ is given below (see Listing~\ref{lst:instance_calo_calo_correlation_condition_vhd}).\\
...
\verb|sliceHigh| & high value of an object slice.\\
\verb|operator| & valid strings are 'true' and 'false' - 'true' for greater/equal-, 'false' for equal-mode of \et comparator.\\
\verb|threshold| & array with requirements for et\_thresholds (16-bit values, hex notation). Valid values depending on the scales of \et.\\
\verb|etaFullRange| & array to set full range of $\eta$ (boolean).\\
...
\verb|etaW2Ignore| & ignore "window"-comparator 2 of $\eta$ (boolean).\\
...
\subsubsection{Calo Muon Correlation condition - Version 3 - template for VHDL-Producer}
A VHDL-template for VHDL-Producer of instantiating\\ \texttt{calo\_muon\_correlation\_condition\_v3.vhd}\\ is given below (see Listing~\ref{lst:instance_calo_muon_correlation_condition_vhd}).\\
...
\verb|etaFullRange| & set full range of $\eta$ (boolean).\\
\verb|etaW1UpperLimit, ...| & limits for "window"-comparators for $\eta$ (4x 16-bit values, hex notation). Valid values depending on the scales for $\eta$.\\
\verb|etaW2Ignore| & ignore "window"-comparator 2 of $\eta$ (boolean).\\
\verb|phiFullRange| & set full range of $\varphi$ (boolean).\\
\verb|phiW1UpperLimit, ...| & limits for "window"-comparators for $\varphi$ (16-bit values, hex notation). Valid values depending on the scales for $\varphi$.\\
...
\subsubsection{Muon Muon Correlation condition - Version 4 - template for VHDL-Producer}
A VHDL-template for VHDL-Producer of instantiating\\ \texttt{muon\_muon\_correlation\_condition\_v4.vhd}\\ is given below (see Listing~\ref{lst:instance_muon_muon_correlation_condition_vhd}).\\
...
\subsubsection{Calo Esums Correlation condition - Version 3 - template for VHDL-Producer}
\textbf{Insert new text !!!}

\subsubsection{Muon Esums Correlation condition - Version 3 - template for VHDL-Producer}
\textbf{Insert new text !!!}
