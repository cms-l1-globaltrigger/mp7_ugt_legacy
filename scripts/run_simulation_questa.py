...
DefaultQuestaSimLibsName = 'questalibs_vivado_v' # generated in /opt/mentor/
...
class Module(object):#module class and nessesary information
    ...
    def make_files(self, sim_dir, view_wave, mp7_tag, menu_path):#makes files for simulation
        ...
        render_template(os.path.join(pkg_dir, 'fdl_pkg_tpl.vhd'), '%s/vhdl/fdl_pkg.vhd' % self.path, replace_map)
        render_template(os.path.join(gtl_fdl_wrapper_dir, 'gtl_module_tpl.vhd'), '%s/vhdl/gtl_module.vhd' % self.path, replace_map)
